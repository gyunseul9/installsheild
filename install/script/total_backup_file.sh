...
#html 폴더가 없으면 생성.
if [ ! -d /backup/${DATE}/html ]
then
/bin/mkdir /backup/${DATE}/html
echo -e [${TIME}]">>> Create html Directory" >> ${LOGFILES}
fi

#html 폴더의 있는 모든 파일을 복사한다.
/bin/cp -rf /var/www/html/* /backup/${DATE}/html/

echo -e [${TIME}]">>> Copy html " >> ${LOGFILES}
...
#복사한 html 폴더를 압축한다.
/bin/tar zcvf ${DATE}_html.tar.gz ./html

echo -e [${TIME}]">>> Compress "${DATE}"_html" >> ${LOGFILES}

#srv 폴더를 삭제한다.
/bin/rm -rf /backup/${DATE}/html

echo -e [${TIME}]">>> Delete html Directory" >> ${LOGFILES}
